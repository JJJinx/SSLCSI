...
def parse_args():
    ...
    parser.add_argument('--ext_pred', help='the dir of saved existence prediction')
    parser.add_argument(
        '--launcher',
...
def main():
    ...
    for file in testlist:
        ...
        if not distributed:
            ...
            outputs = multi_gpu_test(model, data_loader)  # dict{key: np.ndarray} is the output of last mlp layer (without sigmoid or softmax) (that is logit)
        pred = np.argmax(outputs['head5'], 1)+1 # 人数预测因为标签的缘故是从0开始的，所以应该+1
        filename = file.split('/')[-2]+'_'+file.split('/')[-1].split('.')[0]+'.txt'
        ext_pred = np.loadtxt(os.path.join(args.ext_pred,filename))
        final_pred = pred*ext_pred
        #np.savetxt(os.path.join(cfg.work_dir,filename),pred, fmt='%d')
        np.savetxt(os.path.join(cfg.work_dir,filename),final_pred)
