...
class BaseDataSource(object, metaclass=ABCMeta):
    ...
    def get_gt_frame_labels(self):
        """Get all ground-truth labels (categories) for each frame.

        Returns:
            list[int]: categories for all images.
        """

        gt_frame_labels = np.array([data['gt_frame_label'] for data in self.data_infos])
    ...
    def get_csi(self, idx):
        '''the input data file can be .csv, .dat, .mat files
