...
CONFIG_VAL='/code/mmselfsup/configs/benchmarks/classification/csi_widar/sup_resnet_1xb64-coslr-100e.py'
CONFIG_TEST='/code/mmselfsup/configs/benchmarks/classification/csi_widar/sup_resnet_1xb64-coslr-100e.py'
...
DATE=$(date "+%Y%m%d")
WORK_DIR='/output/mocov2_lr2e-5_resnet_adamw-coslr-150e_widaruser3room1r2_JitterPerm_'${DATE}'/'
PORT='29500'
...
CUDA_VISIBLE_DEVICES=${GPU} python -m torch.distributed.launch \
    ...
    --cfg-options data.train.data_source.type='WiFi_Widar_pt' \
...
CUDA_VISIBLE_DEVICES=${GPU} python -m torch.distributed.launch \
    ...
    find_unused_parameters=True \
    dist_params.port=$PORT \
    --work-dir=${WORK_DIR}${FINTUNE_FOLDER} \
    --launcher="pytorch" ${PY_ARGS}

FTDIR=${WORK_DIR}${FINTUNE_FOLDER}
for file in `find ${FTDIR} -name "*.pth"`; do
    MODEL_PTH=$file
done
